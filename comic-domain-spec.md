...
コンテンツの内部 ID は、ページ番号や画像座標から生成しない。
ページ追加、ページ差し替え、単行本修正、縦読み版、海外版などで表示位置が
変わっても、同じ Page / Panel / Bubble を追跡できることを優先する。
...
既存サンプルでは `rain-world-ep01-p01-k01-f01` のような可読IDも使っているが、
長期的には「不変の内部ID」と「人間向け表示参照」を分ける。

## 3.2 Database ID

DB レコードには `cuid()` で生成された一意 ID を使用できる。
ただし、Git 正本や API で参照する Page / Panel / Bubble の内部IDとは
役割を分ける。
...
- Page / Panel / Bubble の正本参照
- Quote / Clip / Pack / Proposal / Share Policy の対象解決に使う
...
Display Ref は人間に読みやすいが、ページ順変更に弱い。
内部の永続参照には Internal ID を使う。

## 3.4 URL Alias

公開済みURLを壊さないため、共有URLは Internal ID と Display Ref の両方を
解決できる alias として扱う。

古い Display Ref が変わった場合でも、可能な範囲で Internal ID へ解決する。
...
| 単位 | 説明 | URL パターン |
|------|------|-------------|
| Page | ページ1つ | `/works/{seriesId}/episodes/{episodeId}/page/{pageRef}` |
| Panel | コマ1つ | `/works/{seriesId}/episodes/{episodeId}/panel/{panelRef}` |
| Quote | 吹き出し1つ | `/quote/{quoteId}` または `/works/{seriesId}/episodes/{episodeId}/quote/{bubbleRef}` |
| Clip | コマ範囲 | `/clip/{clipId}` または `/works/{seriesId}/episodes/{episodeId}/clip/{clipRef}` |
| Reaction | タグ検索 | `/reaction/{tag}` |

これらは viewer 側で SSR レンダリングされ、OGP メタタグを含む。
API 側にも対応エンドポイントが存在する（`openapi.yaml` 参照）。

Reader 内ジャンプには `#p5-2-1` のような fragment を使える。
ただし fragment はサーバーに送られないため、SNS共有とOGP生成の正本URLには
path URL を使う。
...
MVPでは公式Quote / 公式Clip / 公式Reactionを優先し、ユーザー自由Clipの公開は
後回しにする。
...
| `/quote/{quoteId}` | SSR | 引用共有ページ |
| `/clip/{clipId}` | SSR | クリップ共有ページ |
...
| `GET /quotes/{quoteId}` | なし | Quote API |
| `GET /clips/{clipId}` | なし | Clip API |
