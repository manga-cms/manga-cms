...
export class FileContentRepository implements ContentRepository {
    ...
    private loadAll(): void {
        ...
        for (const seriesDir of dirs) {
            ...
            for (const epId of manifest.episodes) {
                ...
                episodes.push({
                    schemaVersion: epData.schemaVersion,
                    editionId: epData.editionId,
                    revisionId: epData.revisionId,
                    id: epData.id,
                    ...
                    purchaseUrl: epData.purchaseUrl,
                    redeemUrl: epData.redeemUrl,
                    metadata: epData.metadata,
            ...
            const series: Series = {
                ...
                publicationType: manifest.publicationType ?? "serial",
                lifecycleStatus: manifest.lifecycleStatus ?? manifest.status,
    ...
    findBubble(
        ...
        const panelBubbles = getPanelBubbles(page, panel);
        const bubble = panelBubbles.find((b) => b.bubbleNumber === bubbleNumber) ?? panelBubbles[bubbleNumber - 1];
