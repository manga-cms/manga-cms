...
export type ImportedBubbleSource = "psd_text_layer";

export interface ImportDetectionMetadata {
...
export interface ImportedBubbleDraft {
    stableRef: string;
    source: ImportedBubbleSource;
    textOriginal: string;
    layerName: string;
    groupPath: string[];
    visible: boolean;
    bbox?: { x: number; y: number; width: number; height: number };
    bubbleType?: DraftBubble["bubbleType"];
    speaker?: string;
    speakerConfidence?: DraftBubble["speakerConfidence"];
    textDirection?: DraftBubble["textDirection"];
    lang?: string;
