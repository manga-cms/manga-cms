...
export function buildEpisodePagesFromDraft(draft: DraftPayload) {
    ...
    return {
        ...
        pages: (d.pages ?? []).map((p) => ({
            pageId: `${d.seriesId}-${d.episodeId}-p${String(p.pageNumber).padStart(2, "0")}`,
            pageNumber: p.pageNumber,
            displayRef: p.displayRef,
            images: { ja: p.imagePath ?? "" } as Record<string, string | undefined>,
            imageId: `${d.seriesId}-${d.episodeId}-p${String(p.pageNumber).padStart(2, "0")}-ja`,
            coordinateSpace: "pixel" as const,
            width: p.width ?? 500,
            height: p.height ?? 760,
            panels: (p.panels ?? []).map((pn) => ({
                panelId: `${d.seriesId}-${d.episodeId}-p${String(p.pageNumber).padStart(2, "0")}-pnl${pn.panelNumber}`,
                panelNumber: pn.panelNumber,
                bbox: {
                    ...(pn.bbox ?? { x: 0, y: 0, width: 100, height: 100 }),
                    imageId: `${d.seriesId}-${d.episodeId}-p${String(p.pageNumber).padStart(2, "0")}-ja`,
                    coordinateSpace: "pixel" as const,
                },
                reactionTags: pn.reactionTags ?? [],
            })),
            bubbles: (p.panels ?? []).flatMap((pn) => (pn.bubbles ?? []).map((b: DraftBubble) => ({
                    bubbleId: `${d.seriesId}-${d.episodeId}-p${String(p.pageNumber).padStart(2, "0")}-pnl${pn.panelNumber}-b${b.bubbleNumber}`,
                    panelId: `${d.seriesId}-${d.episodeId}-p${String(p.pageNumber).padStart(2, "0")}-pnl${pn.panelNumber}`,
                    displayRef: b.shortId ?? `b${b.bubbleNumber}`,
                    ...
                    flags: b.flags,
                    bbox: {
                        ...(b.bbox ?? { x: 0, y: 0, width: 100, height: 40 }),
                        imageId: `${d.seriesId}-${d.episodeId}-p${String(p.pageNumber).padStart(2, "0")}-ja`,
                        coordinateSpace: "pixel" as const,
                    },
                }))).map((bubble, index) => ({ ...bubble, bubbleNumber: index + 1 })),
        })),
    };
}
