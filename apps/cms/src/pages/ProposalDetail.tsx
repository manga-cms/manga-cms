...
export default function ProposalDetail() {
    ...
    const createGitHubHandoff = async () => {
        ...
        setHandoffId("");
        try {
            const handoff = await createProposalGitHubHandoff(proposalId, {
                mode: "triage_issue_comment",
                triage_group_key: `${record?.series_id ?? "series"}/${record?.episode_id ?? "episode"}`,
                title: `${record?.kind ?? "proposal"} · ${record?.series_id ?? ""} / ${record?.episode_id ?? ""}`.trim(),
            });
